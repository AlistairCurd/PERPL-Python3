...
def write_html_report_start(fout, info):
    ...
    title_line1 = (
        "<title>Report on *** Produced by +++ of the PERPL "
    ...
    fout.write("\n<body>\n")
    title_line2 = (
        '<h1 align="center">Report on *** Produced by +++ '
...
def write_rot_2d_html_report(info, symmetries, aiccs, weights, table_values):
    ...
    try:
        ...
        print("open() failed", e)
        print(
            "The html report file could not be opened for writing. Try using"
    ...
    fout.write("<p></p>\n")
    fout.write(
        "<h2>Curves of Best Fit of Simulated Data with a Histogram of "
        "Experimental Data</h2>\n"
    )
    fout.write(
        "<p>A histogram of separations between observations in experimental "
        "fluorescence localisation microscopy superimposed with curves "
        "of best fit for a variety of model data.</p>\n"
    )

    fout.write(
        '<IMG SRC=*** ALT="A histogram of separations between observations '
    ...
    fout.write(
        "<p>This table provides data on the fit of the simulated model "
    ...
    fout.write(
        "<p>Parametric model for distances between localisations on vertices "
    ...
    fout.write(
        "<p>In this model, we take account of the fact that rotationally "
    ...
    fout.write(
        '    <th colspan="3">Settings used for the leaset squares curve fitting</th>\n'
    )
    fout.write("  </tr>\n")
    fout.write("  <tr>\n")
    ...
    fout.write("  	<td>1</td>\n")
    fout.write(
        "   	<td>Diameter of the circle containing the vertices of the "
        "polygon.</td>\n"
    )
    ...
    fout.write("	    <td>2</td>\n")
    fout.write(
        "   	<td>Broadening of the peaks located at distances between "
        "the vertices.</td>\n"
    )
    ...
    fout.write("  	<td>3</td>\n")
    fout.write(
        " 	<td>Amplitude of the contribution of one inter-vertex " "distance.</td>\n"
    )
    ...
    fout.write(" 	<td>4</td>\n")
    fout.write(
        " 	<td>Spread representing the localisation precision for "
    ...
    fout.write(" 	<td>5</td>")
    fout.write(
        " 	<td>Amplitude of the contribution of repeated localisations "
    ...
    fout.write(" 	<td>6</td>\n")
    fout.write(
        " 	<td>Spread of a contribution resulting from unresolvable "
    ...
    fout.write(" 	<td>7</td>\n")
    fout.write(
        " 	<td>Amplitude of the contribution of unresolvable substructure, "
    ...
    fout.write(" 	<td>8</td>\n")
    fout.write(
        " 	<td>Gradient of an isotropic (linearly increasing) background "
        "term.</td>\n"
    )
    ...
    fout.write(" 	<td>9</td>\n")
    fout.write(
        " 	<td>Onset distance for linearly increasing background term, "
    ...
    for index, sym_set in enumerate(table_values):
        ...
        fout.write(
            '     <td rowspan="0"><IMG SRC=*** ALT="Plot of Model '
...
def write_rel_pos_html_report(info):
    ...
    if info["colours_analysed"] == 2:
        report_info = report_info + (
            ...
            + "] to localisations in channel ["
            + repr(info["end_channel"])
            + "]. "
        )
    if info["nns"] > 0:
        report_info = report_info + (
            f"Analysis was restricted to the {info['nns']} " + "nearest neighbour(s). "
        )
    report_info = report_info + (
    ...
    image_line = (
        '<IMG SRC=*** ALT="Scatter plot of localisations in +++ '
    ...
    filename_zoom_scatter_xy = (
        r"scatter_plot_xy_localisations_x" + str(info["zoom"]) + ".png"
    )

    image_zoom_line = (
        '<IMG SRC=*** ALT="Scatter plot of localisations in XY obtained '
    ...
    line = (
        '<IMG SRC=*** ALT="Histogram of +++ separations between observations '
