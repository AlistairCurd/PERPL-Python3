...
def get_input_data(
    infile="S:/Peckham/Bioimaging2/Alistair/Centriole-EPFL"
...
def reconstruct_centrioles(
    centriole_localisation_data,
    index=None,
    plot=True,
    binsize_nm=5.0,
    smoothing_nm=5.0,
    ...
    for i in index:
        ...
        bin_edges_x = np.arange(
            np.min(locs_x) - smoothing_nm * 4.0,
            np.max(locs_x) - smoothing_nm * 4.0,
            binsize_nm,
        )
        bin_edges_y = np.arange(
            np.min(locs_y) - smoothing_nm * 4.0,
            np.max(locs_y) - smoothing_nm * 4.0,
            binsize_nm,
        )
        reconstruction = np.histogram2d(
            locs_x, locs_y, bins=(bin_edges_x, bin_edges_y)
        )[0]
        # Do smoothing by one pixel
        reconstruction = reconstruction.astype(float)
        smoothing_in_pixels = smoothing_nm / binsize_nm
        reconstruction = gaussian_filter(reconstruction, sigma=smoothing_in_pixels)
...
def get_xy_distances(
    localisations_per_centriole,
    loc_precision_filter=10.0,
    filter_distance=1000.0,
):
    ...
    for i, centriole in enumerate(localisations_per_centriole):
        ...
        filtered_locs = centriole[centriole[:, 3] < loc_precision_filter][:, 0:2]
        print(
            "Analysing "
        ...
        print(
            ...
            + repr(i + 1)
            + " of "
            + repr(len(localisations_per_centriole))
            + ",\n"
...
def make_distance_histogram(distance_values, max_distance, normalise=True):
    ...
    if normalise is True:
        hist, bin_edges = np.histogram(
            ...
            bins=bin_edges,
            weights=np.repeat(
                float(max_distance) / len(distance_values), len(distance_values)
            ),
        )

    else:
        hist, bin_edges = np.histogram(distance_values, bins=bin_edges)

    return hist, bin_edges


def make_distance_histograms_per_centriole(
    distances_per_centriole, max_distance, normalise=True
):
    ...
    for distances in distances_per_centriole:
        hist_values, bin_edges = make_distance_histogram(
            distances, max_distance, normalise
        )
...
def plot_95_ci(
    axes, x_values, model, params_optimised, params_covar, vector_input_model
):
    ...
    stdev = models.stdev_of_model(
        x_values, params_optimised, params_covar, vector_input_model
    )
...
def plot_95_ci_internal_bg(
    ...
    stdev_lower = models.stdev_of_model(
        x_values[x_values < bg_dia],
        params_optimised,
        params_covar,
        vector_input_model_below_bg_dia,
    )
    ...
    # Get SD of model at the x-values less than bg_dia
    stdev_higher = models.stdev_of_model(
        x_values[x_values > bg_dia],
...
def centriole_model_xy_distances_9fold_variable_vertices(
    ...
    relative_positions = getdistances(
        vertices, filter_distance, verbose=False, sort_and_halve=False
    )
...
def centriole_model_xy_distances_9fold_variable_vertices_vectorargs(input_vector):
...
def centriole_model_xy_distances_9fold_variable_vertices_internal_bg(
    ...
    separation_values_below_bg_dia = separation_values[separation_values < bg_dia]
    background = bg_amp * pair_correlation_disk(
        separation_values_below_bg_dia, radius=bg_dia / 2.0
    )
...
def centriole_model_xy_distances_9fold_variable_vertices_internal_bg_vectorargs(
    ...
    rpd = rpd + bg_amp * pair_correlation_disk(separation_values, radius=bg_dia / 2.0)
...
def set_up_variable_vertices_model_9fold_no_bg_with_fit_settings():
    ...
    numdifftools.
    """
    variable_vertices_model_9fold_no_bg_with_fit_settings = ModelWithFitSettings(
    ...
    variable_vertices_model_9fold_no_bg_with_fit_settings.param_bounds = bounds
...
def set_up_variable_vertices_model_9fold_internal_bg_with_fit_settings():
    ...
    numdifftools.
    """
    variable_vertices_model_9fold_internal_bg_with_fit_settings = ModelWithFitSettings(
    ...
    variable_vertices_model_9fold_internal_bg_with_fit_settings.param_bounds = bounds
...
def make_fit_results_table_variable_vertices_model_9fold_no_bg(
    ...
    error_on_params_df = pd.DataFrame(
        columns=[
            "centriole_number",
            "diameter",
            "vertices_broadening",
    ...
    for i, _ in enumerate(distance_histograms_list):
        ...
        estimated_params_df["centriole_number"] = estimated_params_df[
            "centriole_number"
        ].astype(int)
        error_on_params_df.loc[i] = table_row_errors
        error_on_params_df["centriole_number"] = error_on_params_df[
            "centriole_number"
        ].astype(int)
...
def select_centrioles_by_error_ratio_threshold_on_peaks(
    ...
    n_peaks_below_threshold = (
        ratios_below_threshold_df.peak1_amp
    ...
    # ratio threshold
    selected_below_threshold = ratios_df.loc[
        n_peaks_below_threshold >= n_peaks_required_below_threshold
    ]
...
def main():
    ...
    outfile_root = (
        outfile_root + "_" + repr(max_localisation_precision) + "nm_precision"
    )

    # Get a list of XY-distances between localisations within each centriole,
    # with one list item per centriole.
    distances_xy_per_centriole = get_xy_distances(
        locs_per_centriole, max_localisation_precision
    )
    ...
    # Mean bin value is 1.
    (distance_histograms_list) = make_distance_histograms_per_centriole(
        distances_xy_per_centriole, max_fit_distance
    )[0]

    outfile_dist_hist_list = outfile_root + "_distance_histograms_normalised.npy"
    ...
    (distance_histograms_list) = make_distance_histograms_per_centriole(
        distances_xy_per_centriole, max_fit_distance, normalise=False
    )[0]

    outfile_dist_hist_list = outfile_root + "_distance_histograms_not_normalised.npy"
    np.save(outfile_dist_hist_list, distance_histograms_list)
    ...
    (estimated_params_df, error_on_params_df, error_to_estimate_ratios_df) = (
    ...
    for i in [0, 26]:
        ...
        fig, axes = plot_distance_histogram(
            distance_histograms_list[i], bin_edges=np.arange(max_fit_distance + 1)
        )
        ...
        axes.set_title(
            ...
            + repr(i)
            + " (0 to"
            + repr(len(distance_histograms_list) - 1)
            + ")"
        )
    ...
    summed_histograms = sum_distance_histograms(
        distance_histograms_list, normalise=True
    )
    ...
    # Plot distance histogram
    fig, axes = plot_distance_histogram(
        summed_histograms, bin_edges=np.arange(max_fit_distance + 1)
    )
