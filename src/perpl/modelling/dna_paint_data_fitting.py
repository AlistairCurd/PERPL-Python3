...
def set_up_tri_prism_on_grid_1_length_2disobg_substruct_with_fit_info():
    ...
    """
    model_with_info = ModelWithFitSettings(
    ...
    model_with_info.initial_params = initial_params
    model_with_info.param_bounds = bounds
...
def fitmodel_to_hist(
    ...
    params_optimised, params_covar = curve_fit(
        ...
        p0=(
            ...
            10.0,  # locamp, locprec
            10.0,
            10.0,  # structamp, spread
            10.0,
            10.0,  # substructamp, substructspread
            200.0,
            100.0,
            10.0,  # gridspace, gridamp, gridspread
            0.001,
        ),  # bgslope
        bounds=(
            [
                ...
                0.0,
                0.0,
                0.0,  # gridspace, gridamp, gridspread
                0.0,
            ],  # bgslope
            [
                200.0,
                200.0,  # 200., # a, b, c
                100.0,
                100.0,  # locamp, locprec
                100.0,
                100.0,  # structamp, spread
                100.0,
                100.0,  # substructamp, substructspread
                400.0,
                1000.0,
                100.0,  # gridspace, gridamp, gridspread
                0.1,
            ],
        ),  # bgslope
    )
    ...
    # Calculate AICc
    ssr = np.sum(
        (model(np.arange(fitlength) + 0.5, *params_optimised) - distancehist) ** 2
    )
...
def plot_xyz_distance_histogram(distances, fitlength, color="gray"):
    ...
    hist_values, bin_edges = histaxes.hist(
        ...
        weights=np.repeat(float(fitlength) / len(distances), len(distances)),
        color=color,
        alpha=0.5,
    )[
        0:2
...
def plot_model_components_tri_prism(
    fitlength,
    side_length,
    locamp,
    locprec,
    ...
    rep_locs_term = locamp * models.pairwise_correlation_3d(
        distance_values, 0.0, np.sqrt(2) * locprec
    )
    plt.plot(distance_values, rep_locs_term)

    # Plot substructure term
    substructure_term = substructamp * models.pairwise_correlation_3d(
        distance_values, 0.0, np.sqrt(2) * locprec
    )
    ...
    tri_prism_peak_1 = (
        18
        * structamp
        * models.pairwise_correlation_3d(distance_values, xyz_distances[0], spread)
    )
    ...
    tri_prism_peak_2 = (
        12
        * structamp
        * models.pairwise_correlation_3d(distance_values, xyz_distances[1], spread)
    )
    plt.plot(distance_values, tri_prism_peak_2)

    # Plot square grid componenet
    square_grid_component = gridamp * models.pairwise_correlation_2d(
        distance_values, gridspace, gridspread
    ) + gridamp * models.pairwise_correlation_2d(
        distance_values, gridspace * np.sqrt(2), gridspread
    )
...
def dna_origami_fig_plot(
    ...
    distance_hist, bin_edges = np.histogram(
        xyz_distances,
        weights=np.repeat(float(fitlength) / len(xyz_distances), len(xyz_distances)),
        bins=bin_edges,
    )
    width = 1.0
    ...
    stdev = stdev_of_model(
        bin_centres, params_optimised, params_covar, model_with_info.vector_input_model
    )
    axes.fill_between(
        bin_centres,
        model_with_info.model_rpd(bin_centres, *params_optimised) - stdev * 1.96,
...
def model_akaike_weights():
    ...
    """
    models = [
        "tri_prism_on_grid_1_length_2disobg_substruct",
    ...
    aiccs = [-1748.52577, -1746.32688, -1477.75366, -1438.53290]
    weights = stats.akaike_weights(aiccs)

    print("Model\t\tAICc\t\tAkaike weight")
    for i in range(len(models)):
        print("{0}\t\t{1:.2f} \t{2:.2}".format(models[i], aiccs[i], weights[i]))
...
def plot_three_models(relpos_xyz, fitlength):
    ...
    xyz_distances = np.sqrt(
        relpos_xyz[:, 0] ** 2 + relpos_xyz[:, 1] ** 2 + relpos_xyz[:, 2] ** 2
    )
    xyz_distances = xyz_distances[xyz_distances < fitlength]
    bin_edges = np.arange(fitlength + 1)
    ...
    axes = fig_histogram.add_subplot(111)
    axes.bar(
        bin_centres, distance_hist, align="center", width=width, color="gray", alpha=0.5
    )
    axes.set_xlabel("Separation (nm)")
    axes.set_ylabel("Counts (normalised)")
