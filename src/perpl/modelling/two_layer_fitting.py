...
def get_input(info):
    ...
    print("The file should contain an array " "with one relative position per row.\n")
    ...
    try:
        fitlength = int(input("What maximum distance would you like " "to set (nm)? "))
    except ValueError:
...
def log_file_header(log_file, info):
    ...
    log_file.write(
        r"This program ran at "
        + info["start"]
        + r" on the host system "
        + info["host"]
        + "."
    )
...
def two_layer_model_constant_bg(
    distance_values_1d,
    layer_separation,
    ...
    within_layer_peak = amplitude_within_layer * pairwise_correlation_1d(
        distance_values_1d, 0, broadening
    )

    between_layers_peak = amplitude_between_layers * pairwise_correlation_1d(
        distance_values_1d, layer_separation, broadening
    )
...
def two_layer_model_exp_decay_bg(
    distance_values_1d,
    layer_separation,
    amplitude_within_layer,
    amplitude_between_layers,
...
def two_layer_model_exp_decay_bg_vectorargs(input_vector):
    ...
    # Evaluate
    rpd = two_layer_model_exp_decay_bg(
        separation_values_1d,
        layer_separation,
        amplitude_within_layer,
        amplitude_between_layers,
...
def fit_two_layer_model(
    experimental_data, model=two_layer_model_constant_bg, fitlength=200.0
):
    ...
    params_optimised, params_covar = curve_fit(
        ...
        experimental_data,
        p0=[
            60.0,  # layer_separation
            10.0,  # amplitude_within_layer
            10.0,  # amplitude_between_layers
            20.0,  # broadening
            # 0.01  # background_offset (constant background)
            10.0,  # bg amplitude (exponential decay background)
            100,  # bg scale parameter (exponential decay background)
        ],
        bounds=(
            0.0,
            [
                120.0,  # layer_separation
                100.0,  # amplitude_within_layer
                100.0,  # amplitude_between_layers
                100.0,  # broadening
                # 0.1  # background_offset (constant background)
                100.0,  # bg amplitude (exponential decay background)
...
def main():
    ...
    dimension_to_analyse = int(
        input(
            "Which column of the relative positions data "
    ...
    params_optimised, params_covar, params_1sd_error = fit_two_layer_model(
        distance_histogram_values, model=model, fitlength=fitlength
    )
    ...
    # Plot model
    axes.plot(
        bin_centres, model(bin_centres, *params_optimised), color="xkcd:red", lw=0.75
    )
    vector_input_model = two_layer_model_exp_decay_bg_vectorargs
    stdev = stdev_of_model(x_values, params_optimised, params_covar, vector_input_model)
    axes.fill_between(
        x_values,
